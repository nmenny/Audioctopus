...
class LinkResource extends Resource {
    ...
    toString() {
        return `Youtube video **${super.toString()}**`
    }
}
...
class ResourceSet {
    ...
    getCurrentResource() {
        if (this.currIdx <= 0 || this.currIdx > this.set.length)
            return undefined
    ...
    next() {
        ...
        if (this.selectRnd) {
            const selectIdx = Math.round(
                Math.random() * (this.lastingSetIdx.length - 1)
            )
            this.currIdx = this.lastingSetIdx[selectIdx]
            this.lastingSetIdx = this.lastingSetIdx
                .slice(0, selectIdx)
                .concat(this.lastingSetIdx.slice(selectIdx + 1))
        } else {
