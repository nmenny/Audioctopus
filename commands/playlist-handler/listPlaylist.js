...
const { createFolderName } = require(
...
const command = new SlashCommandBuilder()
    ...
    .addIntegerOption((opt) =>
        opt
            .setName('from')
            .setDescription(
                'The index to start the listing in the playlist list. (starts at 1)'
            )
            .setMinValue(1)
    )
...
async function execute(interact) {
    ...
    if (!fs.existsSync(folderName)) {
        await interact.reply({
            content: `No playlist in the server.`,
            ephemeral: true,
        })
        return
    }
