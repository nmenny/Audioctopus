...
const { PlayerInfo } = require(
    path.join(__dirname, '..', '..', 'utils', 'types.js')
)
const { FileResource, LinkResource, ResourceSet, checkFileFormat } = require(
    path.join(__dirname, '..', '..', 'utils', 'resourceHandler.js')
)
    ...
    .addSubcommand((scmd) =>
        scmd
            ...
            .addBooleanOption((opt) =>
                opt.setName('loop').setDescription('Plays music in a loop.')
            )
    )
    .addSubcommand((scmd) =>
        scmd
            .setName('from-youtube')
...
function createPlayer(cli, gid) {
    ...
    player.on(AudioPlayerStatus.Idle, async () => {
        ...
        if (playerI && playerI.inLoop) {
            let resource = await playerI.resource.load()
...
async function execute(interact) {
    ...
    switch (interact.options.getSubcommand()) {
        case 'from-file':
            file = interact.options.getAttachment('file')

            if (!checkFileFormat(file.contentType)) {
                await interact.editReply({
                    content: `Incorrect file format. (not one of .mp3, .ogg or .webm).`,
                    ephemeral: true,
                })
                return
            }
            ...
            currRessourceData.addResource(
                new FileResource(file.name, file.attachment)
            )
            currRessourceData.next()
            resource = await currRessourceData.load()
            break
        case 'from-youtube':
            videoLink = interact.options.getString('link')
            ...
            break
        case 'from-playlist':
            currRessourceData.selectRnd =
                interact.options.getBoolean('random') ?? false
            playlist = loadPlaylist(
                interact.guildId,
                interact.options.getString('playlist-name')
            )

            if (!playlist || playlist['list'].length === 0) {
                await interact.editReply({
                    content: `Playlist "${interact.options.getString('playlist-name')}" does not exist or is empty.`,
                    ephemeral: true,
                })
                return
            }
            ...
            for (let music of playlist['list']) {
                currRessourceData.addResource(
                    new LinkResource(music.title, music.link)
                )
            }

            // Checks the id of the music
            musicId = interact.options.getInteger('music-id') ?? 1
    ...
    const newInfo = new PlayerInfo(
        subscription,
        currRessourceData,
        interact.options.getBoolean('loop')
    )
    interact.client.musicPlayers.set(interact.guildId, newInfo)

    await interact.editReply({
        content: `Playing ${currRessourceData.toString()} ${newInfo.inLoop ? '(loop)' : ''} ${currRessourceData.selectRnd ? '(random)' : ''}`,
        ephemeral: true,
    })
}
